...
db_path = r'/home/leo/.mixxx/mixxxdb.sqlite'
conn = sqlite3.connect(db_path)
conn.row_factory = sqlite3.Row
cursor = conn.cursor()

# Configuración: Mapea el path base original (Linux) al nuevo (Windows)
# Cambia OLD_BASE y NEW_BASE según tu necesidad
OLD_BASE = "/home/leo/Música/electronica"  # Path base en Linux (ajusta si es diferente)
NEW_BASE = r"/D:/electronica"   # Nuevo path base en Windows (usa r"" para raw string y evitar escapes)
...
for row in data:
    # ----------------------------------------------------------
    # Procesamos el Location para que funcione en Windows + Rekordbox
    # ----------------------------------------------------------
    original_location = str(row['Location'])  # ej: file://localhost//home/leo/Música/...

    if original_location.startswith("file://localhost//"):
        # Quitamos el prefijo file://localhost// y obtenemos el path absoluto de Linux
        linux_path = original_location[17:]  # 17 caracteres = len("file://localhost//")

        # Remapeamos la carpeta base
        if linux_path.startswith(OLD_BASE):
            windows_path = linux_path.replace(OLD_BASE, NEW_BASE, 1)
        else:
            windows_path = linux_path  # si por algún motivo no coincide, lo dejamos tal cual

        # Cambiamos las barras / → \  (Rekordbox en Windows lo prefiere así)
        # windows_path = windows_path.replace("/", "\\")

        # Reconstruimos la URL en formato que Rekordbox entiende perfectamente
        new_location = f"file://localhost{windows_path}"

    else:
        new_location = original_location  # fallback por si hay algún location raro

    # ----------------------------------------------------------
    # Creamos el elemento TRACK (todo igual que antes, solo cambiamos Location)
    # ----------------------------------------------------------
